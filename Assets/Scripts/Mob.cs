...
public class Mob : IMapObject
{
	...
	public Mob(int mobId, bool isDisable, bool isDontMove, bool isFire, bool isIce, bool isWind, int templateId, int sys, long hp, sbyte level, long maxp, short pointx, short pointy, sbyte status, sbyte levelBoss)
	{
		...
		updateHp_bar();
		per_tem = (int)((double)((long)hp * 100) / maxHp);
		isDie = false;
		xSd = pointx;
		ySd = pointy;
		if (isNewModStand())
	...
	{
		if (!(this is BachTuoc) && !(this is BigBoss2) && !(this is BigBoss))
		{
	...
	public void updateMobAttack()
	{
		...
		{
			if (arrMobTemplate[templateId].type != 0 && !isDontMove && !isIce)
			{
				_ = isWind;
			}
			if (tick == array.Length)
	...
	{
		len = (int)((double)((long)hp * 100) / maxHp * (double)w_hp_bar) / 100;
		per = (int)((double)((long)hp * 100) / maxHp);
		if (per == 100)
		{
			per_tem = per;
		}
